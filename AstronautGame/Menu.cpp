...
Menu::Menu(Camera camera, sf::RenderWindow* window)
{
	...
	menu[0]->setPosition({ xPosition, float(window->getSize().y) / (MAX_NUMBER_OF_ITEMS + 1) * 1});

	menu[1]->setFillColor(sf::Color::White);
	menu[1]->setString("Levels");
	bounds = menu[1]->getLocalBounds();
	menu[1]->setOrigin({ bounds.size.x / 2, bounds.size.y / 2 });
	menu[1]->setPosition({ xPosition, float(window->getSize().y) / (MAX_NUMBER_OF_ITEMS + 1) * 2 });
